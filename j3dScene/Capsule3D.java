...
public class Capsule3D extends Shape3D {
	...
	public Capsule3D(float height, float radius, Appearance app) {
		...
		//Upper half-sphere
		Point3d c = new Point3d(0,height/2,0);
		float r = radius;
		double d = PI/divisions;

		for(double theta=0;theta<PI;theta+=d){
			for(double phi=0;phi<PI;phi+=d){
				verts.add(new Point3d(  c.getX() + r*sin(theta  )*cos(phi  )  ,  c.getY() + r*sin(theta  )*sin(phi  )  ,  c.getZ() + r*cos(theta  )  ));
				verts.add(new Point3d(  c.getX() + r*sin(theta+d)*cos(phi  )  ,  c.getY() + r*sin(theta+d)*sin(phi  )  ,  c.getZ() + r*cos(theta+d)  ));
				verts.add(new Point3d(  c.getX() + r*sin(theta  )*cos(phi+d)  ,  c.getY() + r*sin(theta  )*sin(phi+d)  ,  c.getZ() + r*cos(theta  )  ));
                                                                                   
				verts.add(new Point3d(  c.getX() + r*sin(theta+d)*cos(phi+d)  ,  c.getY() + r*sin(theta+d)*sin(phi+d)  ,  c.getZ() + r*cos(theta+d)  ));
				verts.add(new Point3d(  c.getX() + r*sin(theta  )*cos(phi+d)  ,  c.getY() + r*sin(theta  )*sin(phi+d)  ,  c.getZ() + r*cos(theta  )  ));
				verts.add(new Point3d(  c.getX() + r*sin(theta+d)*cos(phi  )  ,  c.getY() + r*sin(theta+d)*sin(phi  )  ,  c.getZ() + r*cos(theta+d)  ));
		...
		for(Point3d v: verts){ vertArr[i++] = v.getX(); vertArr[i++] = v.getY(); vertArr[i++] = v.getZ(); }
		i=0;
